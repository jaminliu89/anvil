...
- [x] G5.1 系统托盘（关闭窗口最小化到托盘 / 托盘菜单 / 点击显示窗口）
- [x] G5.2 桌面通知（插件已装，接口预留）
- [ ] G5.3 单实例（重复启动聚焦已有窗口）—— 暂搁置，API 兼容性待修
- [x] G5.4 全局快捷键（Option+Space 唤起 / 隐藏）
- [x] G5.5 开机自启（设置项开关 + autostart plugin）
- [x] G5.6 窗口状态记忆（window-state plugin）

## Gate 6 · 体验打磨

- [x] G6.1 会话历史列表与搜索 — 由 DSH Web UI 原生提供
- [ ] G6.2 干活轨迹面板（工具调用实时展示）
- [ ] G6.3 审批卡片（同意 / 拒绝）
- [x] G6.4 自定义助手（创建 / 编辑 / 删除 + 本地持久化）
- [x] G6.5 工作区管理（添加本地文件夹 + store 层）
- [ ] G6.6 插件中心（图形化浏览 / 安装 DSH 插件）
- [x] G6.7 主题切换（设置页深色 / 浅色 / 系统切换）
