// openclaw-adapter — OpenClaw 异步编码 Agent (OpenAI)
// 通过 bridge sidecar 代理调用 openclaw CLI
// 异步任务：创建 → 计划 → 批准 → 执行 → 结果
...
export const openclawAdapter: Adapter = {
  id: 'openclaw',
  name: 'OpenClaw',
  description: 'OpenAI OpenClaw 编码 Agent。OpenAI 官方出品。',
  ...
  async execute(_command: string, args: string): Promise<ExecutionResult> {
    ...
    if (subMatch) {
      ...
      switch (sub) {
        ...
        case 'approve': return approveSession(sid)
        case 'pr': return createPR(sid)
      }
    }

    if (!trimmed) {
      return { type: 'system', content: '用法: /openclaw <任务描述>' }
    }
  ...
  async status() {
    try {
      const res = await fetch(`${BRIDGE}/openclaw/health`, { signal: AbortSignal.timeout(2000) })
      if (res.ok) {
        const data = await res.json().catch(() => ({}))
        return { available: true, healthy: true, message: data.message || 'OpenClaw 可用' }
      }
    } catch { /* fall through */ }
    return { available: false, healthy: false, message: 'OpenClaw 未连接（需 bridge 代理）' }
  },
}

async function createSession(prompt: string): Promise<ExecutionResult> {
  try {
    const res = await fetch(`${BRIDGE}/openclaw/create`, {
      method: 'POST',
    ...
    return { type: 'system', content: `OpenClaw 暂不可用: ${e}` }
  }
}

async function sessionStatus(sid: string): Promise<ExecutionResult> {
  try {
    const res = await fetch(`${BRIDGE}/openclaw/status/${sid}`, { signal: AbortSignal.timeout(5000) })
    const data = await res.json()
...
async function sessionLog(sid: string): Promise<ExecutionResult> {
  try {
    const res = await fetch(`${BRIDGE}/openclaw/log/${sid}`, { signal: AbortSignal.timeout(10000) })
    const data = await res.json()
...
async function approveSession(sid: string): Promise<ExecutionResult> {
  try {
    await fetch(`${BRIDGE}/openclaw/approve/${sid}`, {
      method: 'POST',
...
async function createPR(sid: string): Promise<ExecutionResult> {
  try {
    const res = await fetch(`${BRIDGE}/openclaw/pr/${sid}`, {
      method: 'POST',
